...
"    <div class=\"row\">" +
"      <div class=\"span8\">" +
"        <h4><strong><a href=\"#\">${label}</a></strong></h4>" +
"      </div>" +
"    </div>" +
"    <div class=\"row\">" +
"      <div class=\"span2\">" +
...
"        <p></p>" +
"        <p>" +
"          <i class=\"icon-comment\"></i> <a href=\"#\">${comments} Comments</a> | " +
"          <i class=\"icon-tags\"></i> Tags : " +
"			{{each tags}} "+
"		 		<a href=\"#\"><span class=\"label label-info\">${$value}</span></a>" + 
"			{{/each}}  " +
"        </p>" +
"      </div>" +
"    </div>" +
	...
	$.getJSON(url, function(data) {
		...
		$.each(data.sub_categories, function(key, value) {
			if (value.tags.indexOf(forTag) >=0){
			    $.tmpl( "subCategoryCategory", value ).appendTo( "#main_suggested_categories_list" );
				hitCounter++;
			}
		});
